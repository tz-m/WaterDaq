...
int ProgramDigitizer(int handle, DigitizerParams_t Params, CAEN_DGTZ_DPP_PHA_Params_t DPPParams)
{
    ...
    ret |= CAEN_DGTZ_WriteRegister(handle, 0x8000, 0x004E0115);  // Channel Control Reg (indiv trg, seq readout) ??
    ...
    for(i=0; i<MaxNChannels; i++) {
	    ...
	    ret |= CAEN_DGTZ_ReadRegister(handle, 0x1080+i*0x100, &value);
	    value |= 1UL << 24;
	    ret |= CAEN_DGTZ_WriteRegister(handle, 0x1080+i*0x100, value);
        }
    }

    ret |= CAEN_DGTZ_ReadRegister(handle, 0x810C, &value);
    value |= 1UL << 30;
    value &= ~(1UL << 31);
    ret |= CAEN_DGTZ_WriteRegister(handle, 0x810C, value);

    ret |= CAEN_DGTZ_ReadRegister(handle, 0x811C, &value);
    value |= 1UL << 10;
    ret |= CAEN_DGTZ_WriteRegister(handle, 0x811C, value);
...
int main(int argc, char *argv[])
{
    ...
    for (b = 0; b < MAXNB; b++) {
        ...
        *      DPP parameters        *
        \****************************/
        for(ch=0; ch<MaxNChannels; ch++) {
			DPPParams[b].thr[ch] = 50;   // Trigger Threshold (in LSB)
			DPPParams[b].k[ch] = 6000;     // Trapezoid Rise Time (ns) 
			DPPParams[b].m[ch] = 1000;      // Trapezoid Flat Top  (ns) 
			DPPParams[b].M[ch] = 110000;      // Decay Time Constant (ns) 
			DPPParams[b].ftd[ch] = 800;    // Flat top delay (peaking time) (ns) 
			DPPParams[b].a[ch] = 16;       // Trigger Filter smoothing factor (number of samples to average for RC-CR2 filter) Options: 1; 2; 4; 8; 16; 32
			DPPParams[b].b[ch] = 296;     // Input Signal Rise time (ns) 
			DPPParams[b].trgho[ch] = 496;  // Trigger Hold Off
			DPPParams[b].nsbl[ch] = 6;     //number of samples for baseline average calculation. Options: 1->16 samples; 2->64 samples; 3->256 samples; 4->1024 samples; 5->4096 samples; 6->16384 samples
			DPPParams[b].nspk[ch] = 2;     //Peak mean (number of samples to average for trapezoid height calculation). Options: 0-> 1 sample; 1->4 samples; 2->16 samples; 3->64 samples
			DPPParams[b].pkho[ch] = 960;  //peak holdoff (ns)
