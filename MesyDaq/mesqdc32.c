/*********************************************************************
  ...
  Contents:     MESQDC32 32ch 11..13bit Peak sensing QDC
 
  $Id: mesqdc32.c 5177 2011-08-12 02:26:08Z amaudruz $
*********************************************************************/
#include <stdio.h>
#include <stdint.h>
#include <string.h>
#include "mesqdc32drv.h"
#include "mvmestd.h"

/*****************************************************************/
/*
Read MESQDC32 register value
*/
...
/*
  Read MESQDC32 register value
*/
int mesqdc32_ReadData(MVME_INTERFACE *mvme, uint32_t base, uint32_t *pdata, int *nw32)
{
  int i;
  mvme_set_am(mvme, MVME_AM_A24);
  mvme_set_dmode(mvme, MVME_DMODE_D16);
  *nw32 = regReadReg(mvme, base, MESQDC32_BUF_DATA_LEN);
  mvme_set_am(mvme, MVME_AM_A24);
  mvme_set_dmode(mvme, MVME_DMODE_D32);
  if (*nw32) {
    for (i=0 ; i<*nw32 ; i++) {
      *pdata = mvme_read_value(mvme, base + MESQDC32_EVENT_READOUT_BUFFER);
      pdata++;
    }
    return 0;
  }
  return MESQDC32_EMPTY;
}

/* User calls */
/*****************************************************************/
uint32_t mesqdc32_RegisterRead(MVME_INTERFACE *mvme, uint32_t base, int offset)
{
...
/*****************************************************************/
void mesqdc32_Reset(MVME_INTERFACE *mvme, uint32_t base)
{
  regWriteReg(mvme, base, 0, 0);
}

/*****************************************************************/
void mesqdc32_ResolutionSet(MVME_INTERFACE *mvme, uint32_t base, uint32_t resolution)
{
  uint32_t reg;

  reg = MESQDC32_QDC_RESOLUTION;
...
void mesqdc32_ThresholdSet(MVME_INTERFACE *mvme, uint32_t base, uint32_t channel, uint32_t thres)
{
  ...
  } else if (channel < MESQDC32_MAX_CHANNELS) {
    reg = MESQDC32_THRESHOLD_0 + 2*channel;
...
void mesqdc32_ThresholdGet(MVME_INTERFACE *mvme, uint32_t base, uint32_t channel, uint32_t *thres)
{
  ...
  if (channel < MESQDC32_MAX_CHANNELS) {
    reg = MESQDC32_THRESHOLD_0 + 2*channel;
...
int rcWait(MVME_INTERFACE *mvme, uint32_t base, uint32_t modAdd, int timeout) 
{
  ...
  do {
    stat = regReadReg(mvme, base, MESQDC32_RC_SENT_RTN_STAT);
...
uint32_t mesqdc32_MSCF16_IDC(MVME_INTERFACE *mvme, uint32_t base, uint32_t modAdd)
{
  ...
  regWriteReg(mvme, base, MESQDC32_NIM_BUSY, 0x3);
  ss_sleep(10); // >400us
  // Segt Device address
  regWriteReg(mvme, base, MESQDC32_RC_MODNUM, modAdd);
  ss_sleep(1); // >400us
  // Set command read_ID (6)
  regWriteReg(mvme, base, MESQDC32_RC_OPCODE, 6);
  ss_sleep(1); // >400us
  // Init send request
  regWriteReg(mvme, base, MESQDC32_RC_DATA, 0);
  ss_sleep(10); // >400us
  // Wait for command completion
  if (rcWait(mvme, base, modAdd, 10)) {
    regWriteReg(mvme, base, MESQDC32_NIM_BUSY, 0x0);
    return -1;
  }
  // Read data
  reg = regReadReg(mvme, base, MESQDC32_RC_DATA);
  ss_sleep(1); // >400us
...
uint32_t mesqdc32_MSCF16_Set(MVME_INTERFACE *mvme, uint32_t base, uint32_t modAdd, int subAdd, int data)
{
  ...
  ss_sleep(1); // >400us
  // Set for write (16)
  regWriteReg(mvme, base, MESQDC32_RC_OPCODE, 16);
  ss_sleep(1); // >400us
  // Write subaddress 
  regWriteReg(mvme, base, MESQDC32_RC_ADDRESS, subAdd);
  ss_sleep(1); // >400us
  // Write Data
  regWriteReg(mvme, base, MESQDC32_RC_DATA, data);
  ss_sleep(1); // >400us
  // Wait for command completion
  if (rcWait(mvme, base, modAdd, 10)) {
    // deactivate control bus
...
uint32_t mesqdc32_MSCF16_Get(MVME_INTERFACE *mvme, uint32_t base, uint32_t modAdd, int subAdd, uint32_t *data)
{
  ...
  ss_sleep(1); // >400us
  // Set command read (18)
  regWriteReg(mvme, base, MESQDC32_RC_OPCODE, 18);
  ss_sleep(1); // >400us
  // Set subaddress (module Id in the chain)
  regWriteReg(mvme, base, MESQDC32_RC_ADDRESS, subAdd);
  ss_sleep(1); // >400us
  // Init command
  regWriteReg(mvme, base, MESQDC32_RC_DATA, 0x0);
  ss_sleep(1); // >400us
  // Wait for command completion
  if (rcWait(mvme, base, modAdd, 10)) {
    // deactivate control bus
  ...
  // Read data
  *data = regReadReg(mvme, base, MESQDC32_RC_DATA);
  ss_sleep(1); // >400us
  // deactivate control bus
  regWriteReg(mvme, base, MESQDC32_NIM_BUSY, 0x0);
  return 0;
...
uint32_t mesqdc32_MSCF16_RConoff(MVME_INTERFACE *mvme, uint32_t base, uint32_t modAdd, int onoff)
{
  ...
  if (onoff) {
    // Active RC (3)
    regWriteReg(mvme, base, MESQDC32_RC_OPCODE, 3);
  } else { 
    // Deactive RC (4)
    regWriteReg(mvme, base, MESQDC32_RC_OPCODE, 4);
  }
  // Init command
  regWriteReg(mvme, base, MESQDC32_RC_DATA, 0);
  ss_sleep(1); // >400us
  ...
  // Read data
  printf("RC:%d\n", regReadReg(mvme, base, MESQDC32_RC_DATA));
...
/*****************************************************************/
void mesqdc32_AcqCtl(MVME_INTERFACE *mvme, uint32_t base, uint32_t operation)
{
  //  uint32_t reg;
  
  /*  reg = regRead(mvme, base, MESQDC32_ACQUISITION_CONTROL);  
  switch (operation) {
  case RUN_START:
    regWrite(mvme, base, MESQDC32_ACQUISITION_CONTROL, (reg | 0x4));
    break;
  case RUN_STOP:
    regWrite(mvme, base, MESQDC32_ACQUISITION_CONTROL, (reg & ~(0x4)));
    break;
  case COUNT_ALL_TRIGGER:
    regWrite(mvme, base, MESQDC32_ACQUISITION_CONTROL, (reg & ~(0x08)));
    break;
  default:
    break;
  }
...
/*****************************************************************/
void  mesqdc32_Status(MVME_INTERFACE *mvme, uint32_t base)
{
  printf("================================================\n");
  printf("MESQDC32 at A24 0x%x\n", (int)base);
  printf("Address source       0x6000: 0x%x\n", regReadReg(mvme, base, MESQDC32_ADD_SOURCE));
  printf("Address reg          0x6002: 0x%x\n", regReadReg(mvme, base, MESQDC32_ADD_REG));
  printf("Module_ID            0x6004: 0x%x\n", regReadReg(mvme, base, MESQDC32_MODULE_ID)); 
  printf("FW revision          0x600E: 0x%x\n", regReadReg(mvme, base, MESQDC32_FIRM_REV));
  printf("Data format          0x6032: 0x%x\n", regReadReg(mvme, base, MESQDC32_DATA_LEN_FMT));
  printf("Multi Event mode     0x6036: 0x%x\n", regReadReg(mvme, base, MESQDC32_MULTIEVENT));
  printf("EndOfEvent marker    0x6038: 0x%x\n", regReadReg(mvme, base, MESQDC32_MARKING_TYPE));
  printf("bank_operation       0x6040: 0x%x\n", regReadReg(mvme, base, 0x6040));
  printf("QDC Resolution mode  0x6042: 0x%x\n", regReadReg(mvme, base, MESQDC32_QDC_RESOLUTION));
  printf("QDC Input range      0x6060: 0x%x\n", regReadReg(mvme, base, MESQDC32_INPUT_RANGE));
...
@return 0: OK. -1: Bad
*/
int  mesqdc32_Setup(MVME_INTERFACE *mvme, uint32_t base, int mode)
{
  WORD reg;
  ...
  case 0x0:
    ...
    regWriteReg(mvme, base, MESQDC32_START_ACQ, 0);
    
    // Soft reset to restore defaults (0x6008 W=0)
    regWriteReg(mvme, base, MESQDC32_SOFT_RESET, 0);

    // Set data length format (0x6032 W=2) D32
    regWriteReg(mvme, base, MESQDC32_DATA_LEN_FMT, 2);
    
    // Set multi-event off, (0X6036 w=0) use 0x6034 for enable next event
    regWriteReg(mvme, base, MESQDC32_MULTIEVENT, 0);

    // Enable EoE marker for time stamp (0x6038 W=1)
    regWriteReg(mvme, base, MESQDC32_MARKING_TYPE, 1);
  ...
  case 0x1:
    ...
    reg = regReadReg (mvme, base, MESQDC32_FIRM_REV);
    printf("FW rev: %x\n", reg);
    ...
    // Set qdc resolution (0x6042 W=0[2K])
    regWriteReg(mvme, base, MESQDC32_QDC_RESOLUTION, 0);

    // Gate Generator 0, 1 default d20/w50
    // Input range (0x6060 W=0 0..4V input)
    regWriteReg(mvme, base, MESQDC32_INPUT_RANGE, 0);

    // CTRA 00: (0x6069 W'00:VME frq, external reset disable)
    regWriteReg(mvme, base, MESQDC32_TS_SOURCE, 0);
    
    // CTRA  Reset all counters (0x6090 W 1)
    regWriteReg(mvme, base, MESQDC32_RESET_CTL_AB, 1);
    // NIM gate1 (0x606A W=0),
    // NIM Fast Reset 

    // Reset (0x6034)
    regWriteReg(mvme, base, MESQDC32_READOUT_RESET, 0);

    // Start acquisition (0x603A W=1)
    regWriteReg(mvme, base, MESQDC32_START_ACQ, 1);

    // Read buffer data length (0x6030)
    reg = regReadReg (mvme, base, MESQDC32_BUF_DATA_LEN);
    printf("Data in Fifo: %x  ", reg);

    // Read Data ready (0x603E R==1)
    reg = regReadReg (mvme, base, MESQDC32_DATA_READY);
    printf("Data ready: %i\n", reg);

    // After reading the Fifo (0x0000) 
    // Issue the MESQDC32_READOUT_RESET to allow new event
    // Reset (0x6034)
    // regWriteReg(mvme, base, MESQDC32_READOUT_RESET, 0);
...
int main (int argc, char* argv[]) {
  ...
  uint32_t MESQDC32_BASE  = 0x800000;
  uint32_t data[1000];
  int i, status, t, g;
  int debug = 0;
  ...
  for (i = 1; i < argc; i++) {
    ...
    usage:
      printf("usage: mesqdc32.exe -b Base address \n");
  ...
  //  status = mesqdc32_MSCF16_IDC(myvme, MESQDC32_BASE, m);
  //  if (status > 0) printf("MSCF16[%d]: RC-bus:%x - ID_Code:%d\n", m, status, ((status>>1)&0x7f));

  mesqdc32_Status(myvme, MESQDC32_BASE); 
  if (bRCon<2) {
    mesqdc32_MSCF16_RConoff(myvme, MESQDC32_BASE, m, bRCon);
    printf("RCon/off executed (%d)\n", bRCon);
  }
  if (bTWrite==0 && a>4) {
    mesqdc32_MSCF16_Get(myvme, MESQDC32_BASE, m, a, &(data[0]));
    printf("R Threshold: Mod:%d Address:%d: %d\n", m, a, data[0]);
  }
  if (bTWrite==1 && a>4) {
    mesqdc32_MSCF16_Set(myvme, MESQDC32_BASE, m, a, t);
    mesqdc32_MSCF16_Get(myvme, MESQDC32_BASE, m, a, &(data[0]));
    printf("W Threshold: Mod:%d Address:%d: %d\n", m, a, data[0]);
  }
  if (bGWrite==0) {
    mesqdc32_MSCF16_Get(myvme, MESQDC32_BASE, m, 0, &(data[0]));
    printf("R     Gain: Mod:%d Address0: %d\n", m, data[0]);
    mesqdc32_MSCF16_Get(myvme, MESQDC32_BASE, m, 1, &(data[1]));
    printf("R     Gain: Mod:%d Address1: %d\n", m, data[1]);
    mesqdc32_MSCF16_Get(myvme, MESQDC32_BASE, m, 2, &(data[2]));
    printf("R     Gain: Mod:%d Address2: %d\n", m, data[2]);
    mesqdc32_MSCF16_Get(myvme, MESQDC32_BASE, m, 3, &(data[3]));
    printf("R     Gain: Mod:%d Address3: %d\n", m, data[3]);
  }
  if (bGWrite==1) {
    mesqdc32_MSCF16_Set(myvme, MESQDC32_BASE, m, a, g);                                                
    mesqdc32_MSCF16_Get(myvme, MESQDC32_BASE, m, a, &(data[0]));                                                
...
#if 0
  mesqdc32_Setup(myvme, MESQDC32_BASE, 1);

  for (i=0;i<16;i++) {
    status = mesqdc32_MSCF16_IDC(myvme, MESQDC32_BASE, i);
    if (status > 0) printf("MSCF16[%d]: RC-bus:%d - ID_Code:%d\n", i, status & 0x1, ((status>>1)&0x7f));
  }
  printf("\n");

  for (i=0;i<32;i++) {
    mesqdc32_ThresholdSet(myvme, MESQDC32_BASE, i, 1360);
    mesqdc32_ThresholdGet(myvme, MESQDC32_BASE, i, &(data[0]));
    printf("Threshold: 0, ch:%i Data:0x%x (0x%x)\n", i, data[0], status);
  }
  
  for (i=0;i<16;i++) {
    status = mesqdc32_MSCF16_GainSet(myvme, MESQDC32_BASE, 1, i, 2*i);
    printf("Gain Mod: 0, ch:%i Data:0x%x (0x%x)\n", i, 2*i, status);
  }
  printf("\n");

  for (i=0;i<16;i++) {
    status = mesqdc32_MSCF16_GainGet(myvme, MESQDC32_BASE, 1, i, &(data[0]));
    printf("Gain Mod: 0, ch:%i Data:0x%x (0x%x)\n", i, data[0], status);
  }
  ...
  mesqdc32_Setup(myvme, MESQDC32_BASE, 1);
  // CTRA  Reset all counters (0x6090 W 1)
  mesqdc32_RegisterWrite(myvme, MESQDC32_BASE, MESQDC32_TS_DIVIDER, 10000);
  mesqdc32_RegisterWrite(myvme, MESQDC32_BASE, MESQDC32_RESET_CTL_AB, 1);
  //  mesqdc32_RegisterWrite(myvme, MESQDC32_BASE, MESQDC32_TESTPULSER, 0x6);
  uint32_t reg;
  ...
  for (;;){
    do {
      // Read buffer data length (0x6030)
      reg = mesqdc32_RegisterRead(myvme, MESQDC32_BASE, MESQDC32_BUF_DATA_LEN);
    } while (reg==0);
    if (reg) {
      printf("Data in Fifo: 0x%x\n", reg);
      reg = 0;
      mesqdc32_ReadData(myvme, MESQDC32_BASE, &(data[0]), &reg);
      //      for (i=0;i<reg;i++) {
      //	printf("Data[%2.2i]:0x%08x\n", i, data[i]);
      //      } 
      // Reset (0x6034)
      mesqdc32_RegisterWrite(myvme, MESQDC32_BASE, MESQDC32_READOUT_RESET, 0);
    }
  }
#endif
  ...
  /*
  do {
    status = regRead(myvme, MESQDC32_BASE, MESQDC32_ACQUISITION_STATUS);
    printf("Acq Status1:0x%x\n", status);
  } while ((status & 0x8)==0);
  
  n32read = mesqdc32_dataRead(myvme, MESQDC32_BASE, &(data[0]), n32word);
  printf("n32read:%d\n", n32read);
